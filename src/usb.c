...
***************************************************************/
    ...
    *********************************/
    ...
    #if __GNUC__ >= 9
        // We disable inlining of usb_everdrive_readreg to fix system freezes that occur when
        // compiled with optimisations (-O2) enabled on the modern SDK. Unfortunately, it is not
        // clear why inlining causes these issues. It is possible that inlining the function allows
        // other problematic optimisations to be applied.
    ...
    #else
        // The original Windows SDK does not support the noinline attribute, but also does not
        // appear to have the same inlining issue.
        static void usb_everdrive_readreg(u32 reg, u32* result)
        {
            usb_everdrive_readdata(result, ED_GET_REGADD(reg), sizeof(u32));
        }
