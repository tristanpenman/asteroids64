...
DEFINES := _FINALROM=1 NDEBUG=1 F3DEX_GBI_2=1 N64 LOW_RESOLUTION=0
...
$(ELF): $(O_FILES) $(BUILD_DIR)/$(LD_SCRIPT)
	...
	$(V)$(LD) -L $(BUILD_DIR) -T $(BUILD_DIR)/$(LD_SCRIPT) -Map $(BUILD_DIR)/$(TARGET).map --no-check-sections -o $@ $(O_FILES) -L/usr/lib/n64/nusys $(NUAUDIOLIB) -lnusys  -ln_audio -L/usr/lib/n64 -lultra_rom -lnustd -L$(N64_LIBGCCDIR) -lgcc
